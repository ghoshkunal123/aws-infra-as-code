...
resource "aws_lambda_function" "codepipeline_test" {
...
resource "aws_lambda_permission" "allow_codepipeline_to_call_lambda_test" {
  statement_id  = "AllowExecutionFromCodepipeline"
  action        = "lambda:InvokeFunction"
  function_name = "${aws_lambda_function.codepipeline_test.function_name}"
