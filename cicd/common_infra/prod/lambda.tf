...
resource "aws_lambda_function" "codepipeline_prod" {
...
resource "aws_lambda_permission" "allow_codepipeline_to_call_lambda_prod" {
  statement_id  = "AllowExecutionFromCodepipeline"
  action        = "lambda:InvokeFunction"
  function_name = "${aws_lambda_function.codepipeline_prod.function_name}"
